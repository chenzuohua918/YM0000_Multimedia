package com.semisky.ym_multimedia.photo.view.photoview;
...
import com.semisky.ym_multimedia.photo.view.photoview.RotateGestureDetector.OnRotateListener;
...
public class PhotoView extends ImageView {
	...
	private GestureDetector.OnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
		...
		public boolean onDoubleTap(MotionEvent e) {
			...
			// mTmpMatrix.postRotate(mDegrees, mRotateCenter.x,
			// mRotateCenter.y);
	...
	public void animaTo(Info info, Runnable completeCallBack) {
		if (isInit) {
			...
			// 将图片旋转回正常位置，用以计算
			// mAnimaMatrix.postRotate(-mDegrees, mScaleCenter.x,
			// mScaleCenter.y);
			mAnimaMatrix.mapRect(mImgRect, mBaseRect);
	...
	private static final float SCALE_LARGEST_RATIO = 3.0f;// 放大最大比例
	private static final float SCALE_SMALLEST_RATIO = 0.3f;// 缩小最小比例

	/** 手动缩放 */
	public void handScale(float scaleFactor) {
		float scale = mScale * scaleFactor;
		if (scale > SCALE_LARGEST_RATIO || scale < SCALE_SMALLEST_RATIO) {
			return;
		}
		mScale = scale;
		mAnimaMatrix.postScale(scaleFactor, scaleFactor, getWidth() / 2,
				getHeight() / 2);
		executeTranslate();
	}
