package com.semisky.ym_multimedia.radio.horizontallistview;
...
@SuppressLint("NewApi")
public abstract class AbsHorizontalListView extends
		HorizontalAdapterView<ListAdapter> implements TextWatcher,
	...
	public AbsHorizontalListView(Context context, AttributeSet attrs,
		...
		setChoiceMode(a.getInt(R.styleable.AbsListView_choiceMode,
				CHOICE_MODE_NONE));
	 ...
	 * items. This however assumes that all list items have the same width. If
	 ...
	 * that the {@link android.widget.Adapter} used by this view must implement
	 * the {@link android.widget.Filterable} interface.
	 * 
	 ...
	 * starting from the right of the view.
	 * 
	...
	protected int computeHorizontalScrollOffset() {
		...
		if (firstPosition >= 0 && childCount > 0) {
			if (mSmoothScrollbarEnabled) {
					...
					offset = Math.max(
							firstPosition
									* 100
									- (left * 100)
									/ width
									+ (int) ((float) scrollX / getWidth()
											* mItemCount * 100), 0);
					if (mOnScrollListener != null) {
						mOnScrollListener.onScrollOffset(this, offset);
					}
					return offset;
				}
			} else {
				...
				offset = (int) (firstPosition + childCount
						* (index / (float) count));
				if (mOnScrollListener != null) {
					mOnScrollListener.onScrollOffset(this, offset);
				}
	...
	protected int computeHorizontalScrollRange() {
		...
		if (mSmoothScrollbarEnabled) {
			...
			if (scrollX != 0) {
				...
				result += Math.abs((int) ((float) scrollX / getWidth()
						* mItemCount * 100));
			}
		} else {
			result = mItemCount;
		...
		scrollRange = result;
		if (mOnScrollListener != null) {
			mOnScrollListener.onScrollRange(this, result);
		}
		return result;
	}
	...
	protected float getLeftFadingEdgeStrength() {
		...
		if (count == 0) {
			...
			return left < paddingLeft ? (float) -(left - paddingLeft)
					/ fadeLength : fadeEdge;
		}
	}
	...
	protected float getRightFadingEdgeStrength() {
		...
		if (count == 0) {
			...
			return right > width - paddingRight ? (float) (right - width + paddingRight)
					/ fadeLength
					: fadeEdge;
		}
	}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (clipToPadding) {
			...
			canvas.clipRect(scrollX + paddingLeft, scrollY + paddingTop,
					scrollX + right - left - paddingRight, scrollY + bottom
							- top - paddingBottom);
			...
			if (x != mLastX) {
				...
				if (motionView != null) {
					...
					if (atEdge) {
						...
						overScrollBy(overscroll, 0, scrollX, 0, 0, 0,
			...
			if (x != mLastX) {
				...
				if (overScrollDistance != 0) {
					...
					if (overscrollMode == OVER_SCROLL_ALWAYS
							|| (overscrollMode == OVER_SCROLL_IF_CONTENT_SCROLLS && !contentFits())) {
						if (rawDeltaX > 0) {
							mEdgeGlowLeft.onPull((float) overScrollDistance
									/ getWidth());
							if (!mEdgeGlowRight.isFinished()) {
								mEdgeGlowRight.onRelease();
							}
						} else if (rawDeltaX < 0) {
							mEdgeGlowRight.onPull((float) overScrollDistance
									/ getWidth());
							if (!mEdgeGlowLeft.isFinished()) {
								mEdgeGlowLeft.onRelease();
							}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP: {
			...
			// FIXME not needed bacaues we could not implement strict span
			// (j.m.)
			// if (mScrollStrictSpan != null) {
			// mScrollStrictSpan.finish();
			// mScrollStrictSpan = null;
	...
	private class FlingRunnable implements Runnable {
		...
		public boolean isScrollingInDirectionCompat(OverScroller scroller,
			...
			return !finished
					&& Math.signum(xvel) == Math.signum(finalX - startX)
		...
		public void run() {
			...
			case TOUCH_MODE_FLING: {
				...
				if (delta > 0) {
					...
					delta = Math.min(getWidth() - paddingRight - paddingLeft
							- 1, delta);
				} else {
					...
					// Don't fling more than 1 screen
					delta = Math.max(
							-(getWidth() - paddingRight - paddingLeft - 1),
							delta);
				}
				...
				if (more && !atEnd) {
					...
					mLastFlingX = x;
					ViewCompat
							.postOnAnimation(AbsHorizontalListView.this, this);
				} else {
					endFling();
			...
			case TOUCH_MODE_OVERFLING: {
				...
				if (scroller.computeScrollOffset()) {
					...
					if (overScrollBy(deltaX, 0, scrollX, 0, 0, 0,
						...
						invalidate();
						ViewCompat.postOnAnimation(AbsHorizontalListView.this,
								this);
					}
				} else {
					endFling();
	...
	class PositionScroller implements Runnable {
		...
		PositionScroller() {
			final Context context = getContext();
			mExtraScroll = ViewConfiguration.get(context)
	...
	boolean trackMotionScroll(int deltaX, int incrementalDeltaX) {
		...
		if (spaceToLeft < absIncrementalDeltaX
				|| spaceToRight < absIncrementalDeltaX) {
			fillGap(toRight);
		}
	 ...
	 * 
	 * @param toRight
	 *            true if the scroll is going toRight, false if it is going up
	 */
						...
						if (newPos >= 0) {
							...
							if (selectablePos == newPos) {
								...
								if (mSyncWidth == getWidth()) {
									// If we are at the same width as when we
									// saved state, try
									// to restore the scroll position too.
									mLayoutMode = LAYOUT_SYNC;
								} else {
									// We are not the same width as when the
	...
	class AdapterDataSetObserver extends
			HorizontalAdapterView<ListAdapter>.AdapterDataSetObserver {
		@Override
