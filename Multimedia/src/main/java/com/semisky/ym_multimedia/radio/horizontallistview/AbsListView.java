...
public abstract class AbsListView extends AdapterView<ListAdapter> implements
	 ...
	 * want to implement {@link android.widget.SectionIndexer} if it wishes to
	 * display alphabet preview and jump between sections of the list.
	 * 
	...
	public boolean performAccessibilityAction(int action, Bundle arguments) {
		...
		case AccessibilityNodeInfo.ACTION_SCROLL_FORWARD: {
			if (isEnabled() && getLastVisiblePosition() < getCount() - 1) {
				final int viewportHeight = getHeight() - mListPadding.top
						- mListPadding.bottom;
	...
	static class SavedState extends BaseSavedState {
		...
		public String toString() {
			return "AbsListView.SavedState{"
					...
					+ " viewLeft=" + viewTop + " position=" + position
					+ " width=" + height + " filter=" + filter + " checkState="
					+ checkState + "}";
		}
	...
	protected int computeVerticalScrollRange() {
		...
		if (mSmoothScrollbarEnabled) {
			...
			if (scrollY != 0) {
				...
				result += Math.abs((int) ((float) scrollY / getHeight()
						* mItemCount * 100));
			}
		} else {
			result = mItemCount;
	...
	protected float getTopFadingEdgeStrength() {
		...
		if (count == 0) {
			...
			return top < paddingTop ? (float) -(top - paddingTop) / fadeLength
					: fadeEdge;
		}
	}
	...
	void updateScrollIndicators() {
		...
		if (mScrollDown != null) {
			...
			if (!canScrollDown && count > 0) {
				...
				canScrollDown = child.getBottom() > bottom
						- mListPadding.bottom;
			}
		...
		if (mTouchMode == TOUCH_MODE_SCROLL) {
			...
			if (y != mLastY) {
				...
				if (motionView != null) {
					...
					if (atEdge) {
						...
						overScrollBy(0, overscroll, 0, scrollY, 0, 0, 0,
			...
			if (y != mLastY) {
				...
				if (overScrollDistance != 0) {
					final int scrollY = getScrollY();
					overScrollBy(0, overScrollDistance, 0, scrollY, 0, 0, 0,
							mOverscrollDistance, true);
					final int overscrollMode = getOverScrollMode();
					if (overscrollMode == OVER_SCROLL_ALWAYS
							|| (overscrollMode == OVER_SCROLL_IF_CONTENT_SCROLLS && !contentFits())) {
	...
	public void draw(Canvas canvas) {
		...
		if (mEdgeGlowTop != null) {
			...
			if (!mEdgeGlowBottom.isFinished()) {
				...
				int edgeY = Math.max(height, scrollY
		...
		public void run() {
			...
			case TOUCH_MODE_FLING: {
				...
				if (delta > 0) {
					...
					delta = Math.min(getHeight() - paddingBottom - paddingTop
							- 1, delta);
				} else {
					...
					// Don't fling more than 1 screen
					delta = Math.max(
							-(getHeight() - paddingBottom - paddingTop - 1),
							delta);
				}
				...
				if (atEnd) {
					if (motionView != null) {
						...
						overScrollBy(0, overshoot, 0, scrollY, 0, 0, 0,
