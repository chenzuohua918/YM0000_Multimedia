 ...
 * 
 * <p>
 * See {@link ListView}, {@link android.widget.GridView},
 * {@link android.widget.Spinner} and {@link android.widget.Gallery} for
 * commonly used subclasses of AdapterView.
 * 
...
public abstract class AdapterView<T extends Adapter> extends ViewGroup {
	...
	protected boolean performButtonActionOnTouchDownUnhide(MotionEvent event) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			if ((event.getButtonState() & MotionEvent.BUTTON_SECONDARY) != 0) {
				if (showContextMenuUnhide(event.getX(), event.getY(),
						event.getMetaState())) {
					return true;
				}
			}
	...
	protected float getHorizontalScrollFactorUnhide() {
		if (mHorizontalScrollFactor == 0) {
			...
			mHorizontalScrollFactor = outValue.getDimension(context
					.getResources().getDisplayMetrics());
		}
	...
	protected boolean hasOpaqueScrollbarsUnhide() {
		// TODO I actually do not know what to return... just return false
		// (j.m.)
		return false;
	}
