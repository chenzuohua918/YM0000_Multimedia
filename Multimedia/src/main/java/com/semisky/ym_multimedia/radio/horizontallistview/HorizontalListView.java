...
public class HorizontalListView extends AbsHorizontalListView {
	...
	public HorizontalListView(Context context) {
		this(context, null);
		initOverscrollDistance(context);
	}
	...
	public HorizontalListView(Context context, AttributeSet attrs, int defStyle) {
		...
		if (d != null) {
			// If a divider is specified use its intrinsic width for divider
			// width
			setDivider(d);
		}
	...
	private boolean showingRightFadingEdge() {
		...
		return (lastVisiblePosition < mItemCount - 1) || (bottomOfRightChild < listRight);
	}
	 ...
	 * the width with this ListView's padding and divider heights included. If
	...
	private View fillSpecific(int position, int left) {
		...
		if (!mStackFromRight) {
			...
			// This will correct for the left of the first view not touching the
			// left of the list
			adjustViewsLeftOrRight();
	...
	protected void layoutChildren() {
		...
		try {
			...
			// TODO We can use those accessability methods, we just remove those
			// (j.m.)
	 ...
	 * @param flow
	 *            If flow is true, align top edge to x. If false, align bottom
	 *            edge to x.
	 * @param childTop
	 ...
	 */
	private View makeAndAddView(int position, int x, boolean flow, int childTop, boolean selected) {
		View child;
	 ...
	 * @param flowRight
	 *            If true, align top edge to x. If false, align bottom edge to
	 *            x.
	 * @param childTop
	 *            Left edge where children should be positioned
	 * @param selected
	...
	/**
	 * Sets the selected item and positions the selection x pixels from the top
	...
	private boolean arrowScrollImpl(int direction) {
		...
		int nextSelectedPosition = (direction == View.FOCUS_RIGHT) ? lookForSelectablePosition(selectedPos + 1, true)
		...
		if (selectedView != null && selectedView.hasFocus()) {
			...
			if (direction == View.FOCUS_RIGHT) {
				...
				final int xSearchPoint = (selectedView != null && selectedView.getRight() < listRight)
						? selectedView.getRight() : listRight;
				mTempRect.set(xSearchPoint, 0, xSearchPoint, 0);
			}
	...
	public boolean isOpaque() {
		boolean retValue = (mCachingActive && mIsCacheColorOpaque && mDividerIsOpaque && hasOpaqueScrollbarsUnhide())
				|| super.isOpaque();
		if (retValue) {
			...
			final int listRight = getRight() - (mListPadding != null ? mListPadding.right : getPaddingRight());
	 ...
	 * --------------------------------
	 */
	private void initOverscrollDistance(Context context) {
        final DisplayMetrics metrics = context.getResources().getDisplayMetrics();
        final float density = metrics.density;

        mMaxXOverscrollDistance = (int) (density * 100);
	}
	@Override
	protected boolean overScrollBy(int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX, int scrollRangeY,
			int maxOverScrollX, int maxOverScrollY, boolean isTouchEvent) {
		// TODO Auto-generated method stub
		return super.overScrollBy(deltaX, deltaY, scrollX, scrollY, scrollRangeX, scrollRangeY, mMaxXOverscrollDistance, maxOverScrollY,
				isTouchEvent);
	}
	//--------------------------------
}
