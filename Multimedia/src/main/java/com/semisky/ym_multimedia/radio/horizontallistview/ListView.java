...
public class ListView extends AbsListView {
	...
	public ListView(Context context, AttributeSet attrs, int defStyle) {
		...
		CharSequence[] entries = a
				.getTextArray(R.styleable.ListView_android_entries);
		if (entries != null) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mItemCount > 0
			...
			childState = Compat.combineMeasuredStates(childState,
					Compat.getMeasuredState(child));
	 ...
	 * maxHeight is provided, the measuring will stop when the current width
	 * reaches maxHeight.
	 * 
	...
	private boolean arrowScrollImpl(int direction) {
		...
		int nextSelectedPosition = (direction == View.FOCUS_DOWN) ? lookForSelectablePosition(
				selectedPos + 1, true) : lookForSelectablePosition(
				selectedPos - 1, false);
	...
	private int amountToScroll(int direction, int nextSelectedPosition) {
		...
		if (direction == View.FOCUS_DOWN) {
			...
			while (numChildren <= indexToMakeVisible) {
				...
				addViewBelow(getChildAt(numChildren - 1), mFirstPosition
						+ numChildren - 1);
				numChildren++;
			}
	...
	public boolean isOpaque() {
		boolean retValue = (mCachingActive && mIsCacheColorOpaque
				&& mDividerIsOpaque && hasOpaqueScrollbarsUnhide())
				|| super.isOpaque();
		if (retValue) {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (drawDividers || drawOverscrollHeader || drawOverscrollFooter) {
			...
			final int listBottom = viewBottom - viewTop
					- effectivePaddingBottom + scrollY;
			if (!mStackFromBottom) {
				int bottom = 0;
