...
package com.semisky.ym_multimedia.ymbluetooth.avr;
...
public class AvrcpCommandManager implements
	...
	protected static final String TAG = "FuncBTOperate";
	...
	// 2 = Shuffle attrs
	// 3 = Scan attrs
	List<AvrcAppAttributeWrapper> mAttrsData = new ArrayList<AvrcAppAttributeWrapper>(
			5);
	...
	// Method to initialize all AppSettingAttribute variables in
	// AvrcpCommandManager
	public void initAppSettingAttrs() {
		...
		// BtLogger.d(TAG, "initAppSettingAttrs() size: " + mAttrsData.size());
		for (byte i = 0; i < 4; i++) {
			BtLogger.d(TAG, "initAppSettingAttrs() >>> " + i);
	...
	protected Handler mInitTimeoutHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			// BtLogger.d(TAG, "handleMessage : " + msg);
			switch (msg.what) {
			case INIT_TIMEOUT_EVT:
				BtLogger.d(TAG, "Failed to complete init");
	...
	public void processCmds(int cmd) {
		...
		case CMD_LOAD_APP_SETTING_ATTR:
			if (mIsInitCompleted) {
				// BtLogger.d(TAG, "Init already completed. Hence return");
				return;
			}
			/*
			 * BtLogger.d(TAG, "CMD_LOAD_APP_SETTING_ATTR ::: mListAppSettingAttrs:"
			...
			if (!mListAppSettingAttrs) {
				...
				mInitTimeoutHandler.sendEmptyMessageDelayed(INIT_TIMEOUT_EVT,
						10000); // 10 sec
				...
				// BtLogger.d(TAG,
				// "Completed all cmds for init. Send callback now..");
		...
		case CMD_RESET_APP_SETTING_ATTR:
			// BtLogger.d(TAG, "Stop loading of App attributes.");
			...
			if (callback != null)
				callback.onLoadAttributesFailure();
			mIsInitCompleted = true;
			synchronized (LOCK) {
				mCmdCount = 0;
			}
	...
	public byte[] getSupportedAttrs() {
		...
		if (!attrs.hasArray()) {
			// BtLogger.e(TAG, "No supported Attrs found!");
			return new byte[0];
		}
		byte[] b = new byte[count];
		attrs.get(b, 0, count);
		// BtLogger.d(TAG, "Found array :" + b.length);
		return b;
	}
	...
	public ByteBuffer getSupportedAttrValues(byte attributeId) {
		...
		if (!data.isAttrPresent) {
			BtLogger.d(TAG, "Attribute " + attributeId
		...
		if (DBG) {
			...
			tmpBuff.position(0);
			// BtLogger.d(TAG, "getSupportedAttrValues() Orig size :" + size);
			for (int i = 0; i < size; i++) {
				// BtLogger.d(TAG, i+" :::: " + tmpBuff.get());
			}
		}
	...
	private void getAlltAttrVals(boolean getCurrentVals) {
		ByteBuffer attrs = ByteBuffer.allocate(4);
		int count = 0;
		// BtLogger.d(TAG, "getAlltAttrVals() getCurrentVals?" + getCurrentVals);
		for (AvrcAppAttributeWrapper data : mAttrsData) {
			if (data.isAttrPresent) {
		...
		byte[] b = new byte[count];
		attrs.get(b, 0, count);
		// BtLogger.d(TAG, "Array formed :" + b.length);
	...
	public void onListPlayerApplicationSettingAttributesRsp(
		...
		if (!BluetoothAvrcpController.isSuccess(status)) {
			// BtLogger.d(TAG,
			// "Fail to load ListPlayerApplicationSettingAttributes");
			if (!mIsInitCompleted)
				processCmds(CMD_RESET_APP_SETTING_ATTR);
			return;
		}
		for (int i = 0; i < attributeId.length; i++) {
			// BtLogger.d(TAG, "onListPlayerApplicationSettingAttributesRsp() >> "
			// + attributeId[i]);
			if (!isValidAttribute(attributeId[i])) {
				// BtLogger.d(TAG,
				// "onListPlayerApplicationSettingAttributesRsp() Invalid Attribute:"+
		...
		if (isAllAttributesInvalid) {
			// BtLogger.d(TAG,
			// "All attributes are invalid. Skip loading of App Attributes now.");
			if (!mIsInitCompleted)
				processCmds(CMD_RESET_APP_SETTING_ATTR);
			return;
		}
	...
	public void onListPlayerApplicationSettingValuesRsp(BluetoothDevice target,
		...
		if (!BluetoothAvrcpController.isSuccess(status)) {
			// BtLogger.d(TAG, "Fail to load ListPlayerApplicationSettingValues");
			if (!mIsInitCompleted)
				processCmds(CMD_RESET_APP_SETTING_ATTR);
			return;
		}
		...
		for (int i = 0; i < valueId.length; i++)
			// BtLogger.d(TAG, "listAppSettingVals attr:" + attribute + " --> "
			// + valueId[i]);
			if (!isValidAttribute(attribute)) {
				// BtLogger.d(TAG,
				// "onListPlayerApplicationSettingValuesRsp() Invalid Attribute:"+
				// attribute+". Skipping..");
			} else {
				AvrcAppAttributeWrapper data = mAttrsData
						.get((int) attribute - 1);
		...
		synchronized (LOCK) {
			...
			// else
			// BtLogger.d(TAG,
			// "onListPlayerApplicationSettingValuesRsp() mCmdCount:"+mCmdCount);
		}
	}
	...
	public void onGetCurrentPlayerApplicationSettingValueRsp(
		...
		if (!BluetoothAvrcpController.isSuccess(status)) {
			// BtLogger.d(TAG,
			// "Fail to load GetCurrentPlayerApplicationSettingValue");
			if (!mIsInitCompleted)
				processCmds(CMD_RESET_APP_SETTING_ATTR);
			return;
		}
		dumpAttrList();
		for (int i = 0; i < valueId.length; i++) {
			// BtLogger.d(TAG, "onGetCurrentPlayerApplicationSettingValueRsp() i: " +
			// i
	...
	public void onGetPlayerApplicationSettingAttributeTextRsp(
			BluetoothDevice target, byte[] attributeId, String[] attributeText,
			int status) {
		if (!BluetoothAvrcpController.isSuccess(status)) {
			// BtLogger.d(TAG,
			// "Fail to load GetPlayerApplicationSettingAttributeText");
			if (!mIsInitCompleted)
				processCmds(CMD_RESET_APP_SETTING_ATTR);
			return;
		}
		// BtLogger.d(TAG,
		// "onGetPlayerApplicationSettingAttributeTextRsp() attr size:"
		// + attributeId.length + ", size:" + mAttrsData.size());
		try {
			for (int i = 0; i < attributeId.length; i++) {
				// BtLogger.d(TAG,
				// "getAppAttrText i:"+i+", attr:"+attributeId[i]+" --> "+attributeText[i]);
				if (mAttrsData.isEmpty()
						|| mAttrsData.size() < (int) attributeId[i])
					return;
				AvrcAppAttributeWrapper data = mAttrsData
						.get(attributeId[i] - 1);
			...
			// BtLogger.e(TAG,
			// "Error in onGetPlayerApplicationSettingAttributeTextRsp()", ee);
		}
		if (!mIsInitCompleted) {
			mGetAppSettingAttrsText = true;
			processCmds(CMD_LOAD_APP_SETTING_ATTR);
		}
		// BtLogger.d(TAG, "onGetPlayerApplicationSettingAttributeText() exit");
	}
	...
	public void onGetPlayerApplicationSettingValueTextRsp(
		...
		if (!BluetoothAvrcpController.isSuccess(status)) {
			// BtLogger.d(TAG, "Fail to load GetPlayerApplicationSettingValueText");
			if (!mIsInitCompleted)
				processCmds(CMD_RESET_APP_SETTING_ATTR);
			return;
		}
		// BtLogger.d(TAG, "onGetPlayerApplicationSettingValueTextRsp() attr:"
		// + attributeId + ", size:" + mAttrsData.size());
		for (int i = 0; i < valueId.length; i++)
			// BtLogger.d(TAG, "getAppAttrValText attr:" + attributeId + " --> "
			// + valueId[i] + " = " + valueText[i]);
			if (mAttrsData.isEmpty() || mAttrsData.size() < (int) attributeId)
				return;
		if (!isValidAttribute(attributeId)) {
			// BtLogger.d(TAG,
			// "onGetPlayerApplicationSettingValueTextRsp() Invalid Attribute:"+
			// attributeId+". Skipping..");
		} else {
			AvrcAppAttributeWrapper data = mAttrsData
					.get((int) (attributeId - 1));
	...
	public void onGetElementAttributesRsp(BluetoothDevice target,
		...
		// BtLogger.d(TAG, "onGetElementAttributesRsp() status: " + status);
		if (!BluetoothAvrcpController.isSuccess(status)) {
			// BtLogger.d(TAG, "Fail to load GetElementAttributes");
			if (!mIsInitCompleted)
				processCmds(CMD_RESET_APP_SETTING_ATTR);
			return;
		}
		...
		for (int i = 0; i < attributeId.length; i++) {
			// BtLogger.d(TAG, "Attr: " + attributeId[i] + ", value:" +
			// valueText[i]);
			...
			// else
			// BtLogger.e(TAG, "Unhandled Attribute ID : " + attributeId[i]);
		}
		if(callback != null){
	...
	public void onGetPlayStatusRsp(BluetoothDevice target, int songLength,
		...
		// BtLogger.d(TAG, "onGetPlayStatusRsp()");
	...
	public void onPlayerAppSettingChanged(BluetoothDevice target,
		...
		// if(DBG)
		// BtLogger.d(TAG, "onPlayerAppSettingChanged() attr.len : " +
		// attribute.length +
		// " , value.len:"+value.length);
		for (int i = 0; i < attribute.length; i++) {
			// if(DBG) BtLogger.d(TAG,
			// "onPlayerAppSettingChanged() -> ["+i+"] attr : " +
	...
	public void onSetBrowsedPlayerRsp(BluetoothDevice target,
			int numberOfItems, String[] folderPath, int status) {
	}
	...
	public void onGetItemAttributesRsp(BluetoothDevice target,
			int[] attributes, String[] valueTexts, int status) {
	}

	public void onSearchRsp(BluetoothDevice target, int numberOfItems,
			int status) {
	}
	...
	public void onSetAbsoluteVolumeRsp(BluetoothDevice target, byte volume,
			int status) {
	}
	...
	private void dumpAttrList() {
		/*
		 * for (AvrcAppAttributeWrapper data : mAttrsData) { BtLogger.d(TAG,
		 * "@@@ attr : " + data.attributeID + " , isPresent:" +
		 * data.isAttrPresent); if (data.isAttrPresent) { BtLogger.d(TAG,
